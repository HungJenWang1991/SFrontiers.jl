...
function sfmodel_MoMTest(arg::Vararg; data::DataFrame, ω::Union{Vector, Real, Tuple}=1.0, 
    ...
    if s == "H" 
        ...
        _jlms = sqrt(sig2star) .* exp.(normlogpdf.(rat1) .- normlogcdf.(rat1)) .+ (mustar)
        _bc  = exp.(clamp.(-mustar .+ 0.5*sig2star .+ normlogcdf.(rat1 .- sqrt(sig2star)) .- normlogcdf.(rat1), -500.0, 500.0))
    elseif s == "E" 
        myeps = Resid .- (_porc)*sqrt(σᵤ²) # prod: v-(u-E(u)) - E(u) = v-u;  cost: v+(u-E(u)) + E(u) = v+u
        mustar = -myeps .- σᵥ²/sqrt(σᵤ²) 
        rat1 = mustar/sqrt(σᵥ²)
        _jlms = sqrt(σᵥ²) .* exp.(normlogpdf.(rat1) .- normlogcdf.(rat1)) .+ mustar
        _bc = exp.(clamp.(-mustar .+ 0.5*σᵥ² .+ normlogcdf.(rat1 .- sqrt(σᵥ²)) .- normlogcdf.(rat1), -500.0, 500.0))
    end
   ...
   if verbose
    ...
    pretty_table(test_res;
                 column_labels = test_header,
                 formatters = [fmt__printf("%.5f", collect(2:3))])
    ...
    pretty_table([crit1 crit2 crit3];
                 ...
                 formatters = [fmt__printf("%.5f", collect(1:3))])
    ...
    nofpar = length(_dicMoM[:frontier]) + 2
        ...
        for i in 1:nofpar
            if i <= nofpar-2
                ...
                pv_list[i] = 2 * ccdf(TDist(sT - nofpar), abs(t_stats[i]))
   ...
   if verbose && !testonly
    ...
    pretty_table(est_res;
                 column_labels = est_header,
                 formatters = [fmt__printf("%.5f", collect(2:7))])
