...
function checksyn(::Type{Trun})
    ...
    for k in (:hscale, :σₐ², :idvar, :gamma)
        if  (_dicM[k] !== nothing) 
...
function checksyn(::Type{Half})
    ...
    for k in (:μ, :hscale, :σₐ², :idvar, :gamma)
        if  (_dicM[k] !== nothing)
...
function checksyn(::Type{PFEWHT})
    for k in (:depvar, :frontier, :hscale, :μ, :σᵤ², :σᵥ², :idvar)
        if  (_dicM[k] === nothing) 
...
function checksyn(::Type{PFEWHH})
    for k in (:depvar, :frontier, :hscale, :σᵤ², :σᵥ², :idvar)
        if  (_dicM[k] === nothing) 
...
function checksyn(::Type{PFECSWH})
    for k in (:depvar, :frontier, :σᵤ², :σᵥ², :idvar)
        if  (_dicM[k] === nothing) 
...
function checksyn(::Type{PTREH})
    for k in (:depvar, :frontier, :σₐ², :σᵤ², :σᵥ², :idvar)
        if  (_dicM[k] === nothing) 
...
function checksyn(::Type{PTRET})
    for k in (:depvar, :frontier, :μ, :σₐ², :σᵤ², :σᵥ², :idvar)
        if  (_dicM[k] === nothing) 
...
function sfmodel_MixTable(dof::Real=9999) # not using ::Int64 in order to thow informative error
    ...
    pretty_table(if 1<=dof<=40       # correct instance
                 ...
                 formatters = [fmt__printf("%.3f", collect(2:5))],
   ...
   # check multicollinearity in the dataset (matrix)
    ...
    for i in 1:5
        ...
        if length(themat) > 0 && size(themat, 2) > 1  # check only if have more than 1 var
            ...
            r = rank(themat)  # number of linearly independent columns

            if r != size(themat, 2) # rank deficit means collinearity
                # Use QR with column pivoting to identify which columns are independent
                F = qr(themat, ColumnNorm())
                pivots = sort(F.p[1:r])  # first r pivoted columns are independent
